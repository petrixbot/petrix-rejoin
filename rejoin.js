...
async function menu_AcceptAllFriends() {
    // Get cookie if not loaded yet
    if (!USER_COOKIE) {
        console.clear();
        clearScreen();
        console.log(chalk.gray('⏳ Getting Roblox cookies...'));
    ...
    console.log(chalk.gray(`[${formatDate(Date.now())}] 🔄 Starting accept loop (check every ${CHECK_INTERVAL / 1000}s)...\n`));

    while (!stopRequested) {
        const pendingIds = await roblox_friendsRequest(USER_COOKIE);

        if (pendingIds.length === 0) {
            //console.log(chalk.gray(`[${formatDate(Date.now())}] 💤 No pending requests — next check in ${CHECK_INTERVAL / 1000}s...`));
        } else {
            console.log(chalk.cyan(`[${formatDate(Date.now())}] 📋 Found ${pendingIds.length} pending requests...`));
            for (const id of pendingIds) {
                if (stopRequested) break;
                const accepted = await roblox_acceptFriend(USER_COOKIE, id);
                if (accepted) {
                    totalAccepted++;
                    console.log(chalk.green(`[${formatDate(Date.now())}] ✅ Accepted: ${id} (Total: ${totalAccepted})`));
                } else {
    ...
    cleanup();
    console.log(chalk.bold(`\n📊 Done! Total accepted: ${chalk.green(totalAccepted)}`));
    console.log(chalk.gray('\nBack to menu...'));
    await delay(2000);
...
async function menu_RemoveAllFriends() {
    ...
    // Fetch friend list
    console.log(chalk.gray('⏳ Getting connections list...'));
    const friends = await roblox_getFriends(USER_COOKIE, USER_ID);

    if (friends.length === 0) {
        console.log(chalk.yellow('⚠️ No connections found.'));
        console.log(chalk.gray('\nBack to main menu...'));
        await delay(5000);
        return menu_main();
    }

    console.log(chalk.green(`✅ Found ${chalk.yellow(friends.length)} connections\n`));

    // Confirm before removing
    const { confirm } = await prompts({
        type: 'confirm',
        name: 'confirm',
        message: `Are you sure you want to remove all ${friends.length} connections? This cannot be undone!`,
        initial: false
    });
    ...
    console.log(chalk.gray('\n⏳ Starting remove all connections...'));
    ...
    for (let i = 0; i < friends.length; i++) {
        ...
        if (removed) {
            successCount++;
            console.log(chalk.green(`[${i + 1}/${friends.length}] ✅ Removed: @${friend.name} (${friend.id})`));
        } else {
            failCount++;
            console.log(chalk.red(`[${i + 1}/${friends.length}] ❌ Failed : @${friend.name} (${friend.id})`));
        }
        await delay(1000);
    }

    console.log(chalk.bold(`\n📊 Done! Success: ${chalk.green(successCount)} | Failed: ${chalk.red(failCount)}`));
    console.log(chalk.gray('\nBack to main menu...'));
    await delay(2000);
    return menu_main();
}
...
async function menu_Rejoin2() {
    while (true) {
        ...
        console.log(chalk.bold("\nExternal Server"));
        console.log(`- Place ID       : ${chalk.yellow(EXTERNAL_PLACEID)}`);
...
async function start_Rejoin2() {
    ...
    while (!stopRequested) {
        ...
        if (STATUS_REJOIN) {
            ...
            const isInGame = presence.userPresenceType === 2;
            const isCorrectPlace = EXTERNAL_PLACEID ? presence.rootPlaceId?.toString() === EXTERNAL_PLACEID : true;

            if (!isInGame || !isCorrectPlace) {
                const reason = !isInGame ? 'Not in-game' : 'Wrong game';
                console.log(chalk.yellow(`[${formatDate(Date.now())}] ⏳ ${reason} — Rejoining...`));
            ...
            console.log(chalk.gray(`[${formatDate(Date.now())}] ✅ Still in correct server — next check in ${CHECK_INTERVAL / 1000}s...`));
        }
